...
#if defined(NDEBUG)
# error "Billycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5ab50a72db0af855f91d6a911d4c3ba8cd959774ba4892e94c31d552d48a7b84");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Billycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Billycoin Signed Message:\n";
...
{
    // Billycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 950000); // Billycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Billycoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Billycoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Billycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Billycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x186643c4abe761bc3ecea662b428bd26eb335db198c1c9d8ed474135c4bc911d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "3/12/21 - $1.9 Trillion COVID Relief Bill Passes";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0468cde091abd569de167a38035f6521edacdb347005f2d047f242f16c2454abce61fe89bcc94b78cc718b4ad94829f2f11ab4222868abae5cb941d3db1e11b78b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1615564938;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085528492;

        if (fTestNet)
        {
            block.nTime    = 1615564916;
            block.nNonce   = 389155661;
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x6f4a927578515a88ddaefcc4dbab8a5e7b8712fd808fcbadd8616787ac8df229"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc6, 0xb9, 0xdc }; // Billycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BillycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BillycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BillycoinMiner(CWallet *pwallet)
{
    printf("BillycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("billycoin-miner");
    ...
    try { loop {
        ...
        printf("Running BillycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BillycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BillycoinMiner, pwallet));
}
